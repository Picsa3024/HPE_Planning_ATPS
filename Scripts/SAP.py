...
# Bedolla Fuerte, R. (Aug 28, 2021). SAP (version No 1.1 | last update by Ana Barraza (June 18, 2025)). Cd. Juarez: Foxconn eCMMS S.A. DE C.V. V1.1
    ...
    try:
        ...
        #End ZSD5 File
         ...
         session.findById("wnd[1]/usr/btnSPOP-OPTION1").press()
            ...
            session.findById("wnd[0]").maximize()
            session.findById("wnd[0]/tbar[0]/okcd").text = "zatpresult"
            session.findById("wnd[0]").sendVKey(0)
            session.findById("wnd[0]/usr/radWDS").select()
            session.findById("wnd[0]/usr/radWDS").setFocus()
            session.findById("wnd[0]/tbar[1]/btn[8]").press()
            session.findById("wnd[0]/usr/shell").pressToolbarContextButton("&EXPORT")
            session.findById("wnd[0]/usr/shell").selectContextMenuItem("&PC")
            session.findById("wnd[1]/usr/sub:SAPLSPO5:0101/radSPOPLI-SELFLAG[1,0]").select()
            session.findById("wnd[1]/usr/sub:SAPLSPO5:0101/radSPOPLI-SELFLAG[1,0]").setFocus()
            session.findById("wnd[1]/tbar[0]/btn[0]").press()
            session.findById("wnd[1]/usr/ctxtRLGRAP-FILENAME").text = path()+'\\Files\\zatpresult_woDetails.xls'
            session.findById("wnd[1]/usr/ctxtRLGRAP-FILENAME").caretPosition = 50
            session.findById("wnd[1]/tbar[0]/btn[0]").press()
